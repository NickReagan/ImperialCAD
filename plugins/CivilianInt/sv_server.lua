    ...
    PerformHttpRequest(url, function(statusCode, response, headers)
        if statusCode == 200 then
            ...
            if responseData and responseData.ssn then
                ...
                TriggerClientEvent("ImperialCAD:Client:Notify", src, "No civilian found with this SSN.")
            end
        else
            print("^1[API_ERROR]^7 Failed to retrieve data with status code: " .. statusCode)

            if statusCode == 400 then
                print("^1[API_ERROR]^7 An API error occurred. Please check that your imperial_community_id and imperialAPI convars in your server.cfg are correct.")
            end

            TriggerClientEvent("ImperialCAD:Client:Notify", src, "Failed to retrieve data. Check logs.")
        end
...
AddEventHandler("ImperialCAD:registerVehicleToCAD", function(ssn, vehicleModelName, plate, colorName, makeName, year)
    ...
    if not commId or commId == "" then
        TriggerClientEvent("ImperialCAD:Client:Notify", src, "Community ID is not set. Cannot proceed with registration.")
        return
    end

    if not apiKey or apiKey == "" then
        TriggerClientEvent("ImperialCAD:Client:Notify", src, "API Key is not set. Cannot proceed with registration.")
        return
    end
        ...
        function(statusCode, response, headers)
            if statusCode == 200 then
                ...
                if responseData and responseData.success then
                    TriggerClientEvent("ImperialCAD:Client:Notify", src, "Vehicle registered successfully to CAD.")
                else
                    if responseData and responseData.message == "Invalid Community" then
                        print("^1[API_ERROR]^7 Failed to register vehicle. Invalid Community ID or API Key. Please check that your imperial_community_id and imperialAPI convars in your server.cfg are correct.")
                        TriggerClientEvent("ImperialCAD:Client:Notify", src, "Failed to register vehicle. Invalid Community ID or API Key.")
                    else
                        print("Failed to register vehicle. Response: " .. response)
                        TriggerClientEvent("ImperialCAD:Client:Notify", src, "Failed to register vehicle. Check server logs.")
                    end
                end
            else
                TriggerClientEvent("ImperialCAD:Client:Notify", src, "Failed to register vehicle. Status Code: " .. statusCode)

                print("^1[API_ERROR]^7 Failed to register vehicle. Status Code: " .. statusCode)
            end
        end, 
    ...
    PerformHttpRequest(url, function(statusCode, response)
        if statusCode == 200 then
            ...
            if responseData and responseData.fn and responseData.ln then
                ...
                TriggerClientEvent("ImperialCAD:Client:Notify", src, "Driver License data retrieved successfully.")
            else
                print("Invalid response data: " .. response)
                TriggerClientEvent("ImperialCAD:Client:Notify", src, "No driver license data found for the provided SSN.")
            end
        else
            print("^1[API_ERROR]^7 Failed to retrieve driver license data. Status Code: " .. statusCode)

            if statusCode == 400 then
                print("^1[API_ERROR]^7 An API error occurred. Please check that your imperial_community_id and imperialAPI convars in your server.cfg are correct.")
            end

            TriggerClientEvent("ImperialCAD:Client:Notify", src, "Failed to retrieve driver license data. Status Code: " .. statusCode)
        end
    end, "POST", jsonData, headers)
end)
...
AddEventHandler("giveDriverLicenseData", function(ssn, nearestPlayer, sourcePlayerId)
    ...
    if not commId or commId == "" then
        TriggerClientEvent("ImperialCAD:Client:Notify", src, "Community ID is not set. Cannot proceed.")
        return
    end

    if not headers["APIKEY"] or headers["APIKEY"] == "" then
        TriggerClientEvent("ImperialCAD:Client:Notify", src, "API Key is not set. Cannot proceed.")
        return
    end

    if not ssn or ssn == "" then
        TriggerClientEvent("ImperialCAD:Client:Notify", src, "Invalid or missing SSN. Please provide a valid SSN.")
        return
    end

    if not nearestPlayer or nearestPlayer == 0 then
        TriggerClientEvent("ImperialCAD:Client:Notify", src, "No player nearby to give ID, Are they close enough?")
        return
    end
    ...
    PerformHttpRequest(url, function(statusCode, response)
        if statusCode == 200 then
            ...
            if responseData and responseData.fn and responseData.ln then
                ...
                TriggerClientEvent("ImperialCAD:Client:Notify", src, "Driver License data sent successfully.")
                TriggerClientEvent("ImperialCAD:Client:Notify", nearestPlayer, "Driver License data received.")
            else
                print("Invalid response data: " .. response)
                TriggerClientEvent("ImperialCAD:Client:Notify", src, "No driver license data found for the provided SSN.")
            end
